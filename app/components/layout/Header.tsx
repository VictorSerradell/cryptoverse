...
declare global {
  ...
  namespace JSX {
    interface IntrinsicElements {
      "w3m-button": Record<string, unknown>;
    }
  }
}
    ...
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-[rgba(2,2,7,0.85)] border-b border-white/[0.08] px-8 h-16 flex items-center justify-between gap-6">
      ...
      <div className="flex-1 max-w-sm relative">
        ...
        <input
          ...
          className="w-full glass rounded-xl py-2 pl-9 pr-4 text-sm font-sans outline-none
                     focus:border-indigo-500 focus:bg-white/[0.06] transition-all
      ...
      <div className="flex items-center gap-3 shrink-0">
        {/* Live badge */}
        <div
          className="hidden sm:flex items-center gap-2 text-xs font-mono text-slate-500
                        glass px-3 py-1.5 rounded-md"
        >
        ...
        <button
          ...
          className="hidden sm:flex items-center gap-1.5 text-xs font-semibold
                     glass px-3 py-2 rounded-lg text-slate-400
                     hover:text-white hover:border-indigo-500 transition-all"
        >
          <RefreshCw size={13} className={spinning ? "animate-spin" : ""} />
          Refresh
        </button>

        {/* Wallet Connect — Web3Modal auto-styled button */}
        <w3m-button />
      </div>
    </header>
  );
